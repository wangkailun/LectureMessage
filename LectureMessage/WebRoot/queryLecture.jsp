<%@ page language="java" import="java.util.*,java.sql.*,com.db.OrderDB" contentType="text/html; charset=utf-8"%>
<%
    		  ...
    		  %>
    		  	<div class="card">
    		  		...
    		  		<%
    		  			...
    		  			result = order.getOrderStatic(rs.getInt("id"),username);
    		  			if(result == false){
    		  		 %>
    		  		 <a href="servlet/OrderServlet?id=<%=rs.getInt("id") %>&username=<%=session.getAttribute("username") %>&action=order" class="delete">预约</a>
    		  		 <%
    		  		 	}
    		  		 	else
    		  		 	{
    		  		  %>
    		  		  <a href="servlet/OrderServlet?id=<%=rs.getInt("id") %>&username=<%=username %>&action=del" class="delete1">取消预约</a>
    		  		  <%
    		  		  }
    		  		   %>
    		  	</div>
